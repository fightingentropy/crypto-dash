...
export default function M2Chart() {
  ...
  useEffect(() => {
    ...
    const fetchM2Data = async () => {
      ...
      try {
        ...
        const cacheKey = `m2-data-${years}y`;
        ...
        // Fetch fresh data
        const response = await fetch(`/api/m2-money-supply?years=${years}`);
        
        if (!response.ok) {
          throw new Error(`API Error: ${response.status}`);
        }

        const data: M2Response = await response.json();
        setM2Data(data);
  ...
  const getLatestValue = () => {
    if (!m2Data || m2Data.data.length === 0) return null;
    const latest = m2Data.data[m2Data.data.length - 1];
    return {
  ...
  return (
    <div className="rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex flex-col bg-white dark:bg-gray-800 transition-colors">
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-xl font-bold text-gray-900 dark:text-white">M2 Money Supply</h3>
          {latestValue && (
            <div className="mt-1">
              <span className="text-2xl font-bold text-blue-600 dark:text-blue-400">{latestValue.value}</span>
        ...
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center bg-white dark:bg-gray-800 bg-opacity-75 dark:bg-opacity-75 z-10">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-500 dark:text-gray-400 text-sm">Loading M2 data...</span>
            </div>
          </div>
        )}
